...
class TrustEngineVisualizer:
    ...
    def plot_confusion_matrices(self, trained_models: Dict, test_data: Optional[pd.DataFrame] = None,
        ...
        try:
            ...
            for idx, (model_name, model_info) in enumerate(trained_models.items()):
                ...
                try:
                    # Generate sample confusion matrix if no test data
                    if test_data is not None and hasattr(model_info, 'predict'):
                        # Use actual predictions
                        X_test = test_data.drop(['Label'], axis=1)
                        y_test = test_data['Label']
                        y_pred = model_info.predict(X_test)
            ...
            plt.savefig(output_path, **self.figure_config)
    ...
    def plot_classifier_comparison(self, performance_data: Dict,
        ...
        Args:
            performance_data: Dictionary with performance metrics for each classifier
            output_dir: Directory to save plots
            show: Whether to display plots
    ...
    def create_comprehensive_dashboard(self, evaluation_results: Dict,
        ...
        try:
            ...
            for category in risk_categories:
                ...
                category_risks = []
                for model in models:
                    model_results = evaluation_results.get(model, {})
                    risk_summary = model_results.get('trust_score_distribution', {}).get('risk_level_summary', {})
                    category_risks.append(risk_summary.get(category, np.random.uniform(10, 30)))

                avg_risks.append(np.mean(category_risks) if category_risks else 0)
            ...
            ax8.set_title('Average Risk Level Distribution')
